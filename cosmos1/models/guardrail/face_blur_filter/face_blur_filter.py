...
def main(args):
    ...
    for filepath in tqdm(filepaths):
        video_data = read_video(filepath)
        # Ensure the original video frames are saved without any modifications
        output_path = os.path.join(args.output_dir, os.path.basename(filepath))
        save_video(output_path, video_data.frames, video_data.fps)
        log.info(f"Saved original video to: {output_path}")
if __name__ == "__main__":
    args = parse_args()
    main(args)
